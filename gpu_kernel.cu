...
// The main GPU Kernel
extern "C" __global__ void search_seeds(uint64_t* seeds_tested, uint64_t* seeds_found, const uint16_t* dummy_input, int total_candidates, const uint8_t* target_address, int worker_id, int total_workers, int match_mode, int match_prefix_len)
{
    /// Calculate local thread index
    int64_t local_idx = (int64_t)blockIdx.x * blockDim.x + threadIdx.x;

// Calculate global seed index based on worker ID
    const int64_t WORKER_SPACE = (2048LL * 2048LL * 2048LL * 2048LL) / total_workers;
// unlimited number of workers
    int64_t global_idx = local_idx + (worker_id * WORKER_SPACE);

// Early exit if global_idx is beyond total search space
    if (global_idx >= (2048LL * 2048LL * 2048LL * 2048LL)) return;

    uint16_t word_indices[12];
    uint8_t mnemonic[256] = {0};
    int mnemonic_len = 0;

    // Hardcoded known first 8 words
    word_indices[0] = 0; // "apple" etc.
    word_indices[1] = 1;
    word_indices[2] = 2;
    word_indices[3] = 3;
    word_indices[4] = 4;
    ...
    // Now calculate the 4 brute-forced words
    int wordlist_size = 2048;
    int w1 = (global_idx / (wordlist_size * wordlist_size * wordlist_size)) % wordlist_size;
    int w2 = (global_idx / (wordlist_size * wordlist_size)) % wordlist_size;
    int w3 = (global_idx / wordlist_size) % wordlist_size;
    int w4 = global_idx % wordlist_size;
    ...
    if (!validate_bip39_checksum(word_indices)) {
        atomicAdd((unsigned long long int*)seeds_tested, 1ULL);
        return;
    }

    // Build mnemonic text
    for (int i = 0; i < 12; i++) {
        const char* word = wordlist[word_indices[i]];
        for (int j = 0; j < 10; j++) {
            if (word[j] == '\0') break;
            mnemonic[mnemonic_len++] = word[j];
        }
        if (i != 11) {
    ...
    uint8_t seed[64];
    pbkdf2_hmac_sha512(mnemonic, mnemonic_len, (const uint8_t*)"mnemonic", 8, 2048, seed, 64);
    ...
    // Generate real public key
    uint64_t priv_scalar[4];
    #pragma unroll
    for (int i = 0; i < 4; i++) {
    priv_scalar[i] = ((uint64_t)private_key[i * 8 + 0] << 56) |
...
pubkey_to_eth_address(eth_address, pubkey_x, pubkey_y);
...
if (match_mode == 0) {
    ...
    if (match) {
		...
		if (match_mode == 1) printf("[PARTIAL MATCH] ");
        if (match_mode == 2) printf("[ZERO ADDRESS] ");

        atomicAdd((unsigned long long int*)seeds_found, 1ULL);
    }
